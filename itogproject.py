...
# Применение фильтров
def apply_filters():
    genre = combo_filter_genre.get().strip() if combo_filter_genre.get() else None
    year = entry_filter_year.get().strip() if entry_filter_year.get() else None
...
tree = ttk.Treeview(tab_main, columns=columns, show="headings", height=10)
for col in columns:
...
genres = ["", "Драма", "Комедия", "Фантастика", "Боевик", "Ужасы", "Мелодрама"]  # Пустой элемент для сброса фильтра
combo_filter_genre = ttk.Combobox(tab_filter, values=genres, state="readonly")
combo_filter_genre.grid(row=0, column=1, padx=5, pady=5)
